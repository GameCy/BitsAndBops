...
void	Hermite::init(float duration, int numSamples, int numpoints, float* xyz=0)
{
	...
	while(numpoints)
	{
		ControlPoints.push_back( Vec( xyz[0], xyz[1], xyz[2]) );
		xyz+=3;
		numpoints--;
	}
...
Vec	Hermite::CalcPoint(float w)
{
	return Interpolate( w*float(ControlPoints.size()) );
}

// do the interpolation, t= 0 .. (n-1)
Vec	Hermite::Interpolate(float t)
{
	if ( ControlPoints.size()<3 || Tangents.size()<2 )	return Vec(0,0,0);

	int segment = (int) t;
	float s = t- (float)segment;

	if ( (segment+2) > ControlPoints.size())	return Vec(0,0,0);	// numseg=nump-1; need to have segment<=numseg;

	return RawCurveCalc(  ControlPoints[segment], ControlPoints[segment+1], Tangents[segment], Tangents[segment+1], s );
}

Vec Hermite::RawCurveCalc(Vec &point1, Vec &point2, Vec &tangentP1, Vec &tangentP2, float weight)
{
...
Vec	Hermite::GetPoint(float t)
{
	if (SamplePoints.size()<1)	return Vec(0,0,0);
